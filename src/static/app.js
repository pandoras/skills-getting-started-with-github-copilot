  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Build participants HTML (include remove button per participant)
        const participantsHtml =
          details.participants && details.participants.length
            ? `<ul class="participants-list">${details.participants
                .map((p) => `<li><span class="participant-email">${p}</span><button class="remove-btn" data-activity="${name}" data-email="${encodeURIComponent(p)}" title="Remove participant">✖</button></li>`)
                .join("")}</ul>`
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        // Refresh activities to reflect the new state
        fetchActivities();
      } else {
